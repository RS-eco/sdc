...
#' author: "RS-eco"
#' ---
build_ch2018_bioclim_che <- function(filedir = "inst/extdata"){
  rm(list=ls()); gc()
  library(terra); library(dismo); library(processNC); 
  ...
  che <- geodata::gadm(country='CHE', level=0, path = paste0(filedir, "GADM"))
  che <- sf::st_as_sf(che)
