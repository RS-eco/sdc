...
build_corine_cha_che <- function(filedir){
  ...
  cha_dat <- terra::rast(cha_files)
  cha_dat
  
  # Re-project outline data
  che <- sf::st_transform(che, sf::st_crs(cha_dat))
  
  corine_cha_che <- terra::mask(terra::crop(cha_dat, che), che)
  corine_cha_che <- as.data.frame(corine_cha_che, xy=T); rm(cha_dat); gc()
  colnames(corine_cha_che) <- c("x", "y", "9000_00", "9000_90", "0006_00", "0006_06", "0612_06", "0612_12", "1218_12", "1218_18")
  
  head(corine_cha_che)
  tail(corine_cha_che)
  c#orine_cha_che <- corine_cha_che %>% mutate_at(vars(-c(x,y)), function(x) factor(x, levels = c(1:44, 48, 128), 
